...
main =
    program <|
        describe "Any (best case: first true)" <|
            [ lists
            , hamtArrays
            , uint8Arrays
            , float64Arrays
...
lists =
    Constants.sizeScales
        ...
        |> List.map (\( size, list ) -> ( toString size, \_ -> List.any (\x -> x == 0) list ))
        |> scale "List"
...
hamtArrays =
    Constants.sizeScales
        ...
        |> List.map (\( size, array ) -> ( toString size, \_ -> arrayAny (\x -> x == 0) array ))
        |> scale "Hamt Array"
...
uint8Arrays =
    Constants.sizeScales
        ...
        |> List.map (\( size, array ) -> ( toString size, \_ -> JsTypedArray.any (\x -> x == 0) array ))
        |> scale "Uint8 Array"
