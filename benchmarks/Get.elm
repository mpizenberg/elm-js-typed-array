...
lists =
    Constants.sizeScales
        ...
        |> List.map (\( size, list ) -> ( toString size, \_ -> get (size // 2) list ))
        |> scale "List"
...
hamtArrays =
    Constants.sizeScales
        ...
        |> List.map (\( size, array ) -> ( toString size, \_ -> Array.get (size // 2) array ))
        |> scale "Hamt Array"
...
uint8Arrays =
    Constants.sizeScales
        ...
        |> List.map (\( size, array ) -> ( toString size, \_ -> JsTypedArray.getAt (size // 2) array ))
        |> scale "Uint8 Array"
