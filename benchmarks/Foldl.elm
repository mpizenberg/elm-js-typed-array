...
lists =
    Constants.sizeScales
        ...
        |> List.map (\( size, list ) -> ( toString size, \_ -> List.foldl (\x acc -> x + acc) 0 list ))
        |> scale "List"
...
hamtArrays =
    Constants.sizeScales
        ...
        |> List.map (\( size, array ) -> ( toString size, \_ -> Array.foldl (\x acc -> x + acc) 0 array ))
        |> scale "Hamt Array"
...
uint8Arrays =
    Constants.sizeScales
        ...
        |> List.map (\( size, array ) -> ( toString size, \_ -> JsTypedArray.foldl (\x acc -> x + acc) 0 array ))
        |> scale "Uint8 Array"
